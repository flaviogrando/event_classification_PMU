...
dir_dados = 'D:\Google Drive\MATLAB\FINAIS\Dados\Eventos\DFT\';
%dir_modelos = 'C:\DADOS_ATP\Modelos\42_DFT\';
dir_resultados  = 'D:\MEGAsync\DADOS\Resultados\42_DFT_varTVE\';


num_folds = 5;

%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
col = [4];                               %  SELECIONA COLUNA DE SAÍDAS
...
%                     clc
                ...
                result = strcat(['T42-',classificador{c},'-g',num2str(col(cl)),'-e',num2str(erro(f)),'-k',num2str(k(i)),'.mat']); % 
                save(strcat(dir_resultados,result),'time')%,'Yestimado','Ytest','Ztest')
                
                % 6) Salva modelo
%                 Modelo (M), caracterísica (28), fold (1-5),grupo de
%                 classes (1,2 ou 4), classificador
%                 modelo = strcat(['M42-',classificador{c},'-g',num2str(col(cl)),'-e',num2str(erro(f)),'-k',num2str(k(i)),'.mat']); %
