...
dir_dados = 'C:\Users\flavi\Google Drive\MATLAB\FINAIS\Dados\Eventos\DFT_60dB_triang\';
dir_modelo = 'C:\MEGAsync\DADOS\Modelos\DFT_60dB_triang\';
dir_resultado = 'C:\MEGAsync\DADOS\Resultados\freq_DFT_60dB_triang_k2_varFE\';
...
for e=1:length(erro)
    for f=1:length(freqs) % varre parâmetros
        ...
        for cl = 2 %:length(col)
            ...
            [entradas,saidas,labels] = features_extraction_varFE(dir_dados,dados,tipo,freqs(f),col(cl),erro(e),k(1)); 

            for c = [2 3]%1:length(classificador)
