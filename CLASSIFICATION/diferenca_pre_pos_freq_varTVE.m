...
function [D] = diferenca_pre_pos_freq_varTVE(matriz_entrada, f,erro,k)
% D = diferença
% M = magnitude
% A = angulo
% T = tensão
...
% INSERE ERRO
                              % nivel max de erro em percentual


% erro percentual
% vmrand = von mises distribution normalizada por pi = (-1 1)
variancia = erro*(vmrand(0,k,[M,N])/pi); % erros faixa (-1 1)


% figure, hold on, grid on, plot(variancia(:,1),'o')
% figure, hold on, grid on, histogram(variancia(:,1))
